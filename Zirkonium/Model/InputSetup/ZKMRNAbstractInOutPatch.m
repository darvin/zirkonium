...
+(NSArray*)keysToBeCopied
{
	...
	{
		keysToBeCopied = [[NSArray alloc] initWithObjects:@"name", @"numberOfChannels", @"channels", nil];
	}
...
{
	NSMutableArray* channelDicts = [NSMutableArray array];
	NSArray* channels = [[self valueForKey: @"channels"] allObjects];
	NSManagedObject* aChannel;
	for(aChannel in channels) {
		[channelDicts addObject: [(ZKMRNAbstractInOutPatchChannel*)aChannel dictionaryWithValuesForKeys:[NSArray arrayWithObjects:@"sourceChannel", @"patchChannel", nil]]];
	}
	return [NSDictionary dictionaryWithObjectsAndKeys:
					[self valueForKey:@"name"], @"name",
					[self valueForKey:@"numberOfChannels"], @"numberOfChannels",
					channelDicts, @"channels", nil]; 
}
