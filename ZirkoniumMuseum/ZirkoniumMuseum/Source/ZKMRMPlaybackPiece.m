...
{
	//NSLog(@"Init Playback Piece: %@", [absoluteURL path]);
...
{
	unsigned mm, ss, ms; 
	SecondsToMMSSMS([self duration], &mm, &ss, &ms);
	NSString* minutes = (mm < 10) ? [NSString stringWithFormat:@"0%d", mm] : [NSString stringWithFormat:@"%d", mm];
	NSString* seconds = (ss < 10) ? [NSString stringWithFormat:@"0%d", ss] : [NSString stringWithFormat:@"%d", ss];
	NSString* value = [NSString stringWithFormat:@"%@:%@", minutes, seconds];
	return value; 
}
...
- (void)setCurrentPosition:(float)pos;
{
	...
	//NSLog(@"Set Time: %f", pos * [self duration]);
...
- (void)tick:(id)timer
{
	//NSLog(@"Tick with: %f", [[_system clock] currentTimeSeconds]);
