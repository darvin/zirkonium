...
static inline float ZKMRLWarpColorValue(float colorValue) { return powf(colorValue, 4); }
...
- (void)defaultLampOrder
{
	// This is the default lamp order for the Kubus
...
{
	NSError* errorObject;
	if (![self setAddress: address error: &errorObject])
		[[NSApplication sharedApplication] presentError: errorObject];
}

- (BOOL)setAddress:(NSString *)address error:(NSError **)errorObject
{
	if (_address) {
		[_address release];
...
//		_address = @"192.168.230.166";	
	...
	if (kCFSocketSuccess != error) {
		if (kCFSocketError == error && errorObject) {
			...
			*errorObject = [NSError errorWithDomain: NSPOSIXErrorDomain  code: error userInfo: userInfo];
		} else {
			NSDictionary* userInfo = 
			[NSDictionary 
				dictionaryWithObjectsAndKeys: 
	...
	if (kCFSocketSuccess != error) {
		connectedToLanbox = NO;
		return connectedToLanbox;
	}

	[self sendPassword];
	
	connectedToLanbox = YES;
	
	return connectedToLanbox;
}

//- (NSString *)defaultLanBoxAddress { return @"192.168.230.166"; }
- (NSString *)defaultLanBoxAddress { return @"192.168.1.77"; }

- (NSArray *)lampOrder { return _lampOrder; }
- (void)setLampOrder:(NSArray *)lampOrder
{
	if (_lampOrder) [_lampOrder release];
	_lampOrder = [lampOrder copy];
	unsigned numberOfLiveLamps = 0;
	for (NSNumber* order in _lampOrder) {
		if ([order intValue] >= 0) ++numberOfLiveLamps;
	}
	_numberOfChannels = numberOfLiveLamps * 3;
}
...
- (void)tick:(id)sender
{
	...
	if (!debugCount) 
		NSLog(@"DEBUG: Preparing DMX Send");
#endif
...
#if DEBUG_DMX
	if (!debugCount) 
		NSLog(@"DEBUG: Updating light mixer");
#endif
...
#if DEBUG_DMX	
	if (!debugCount) 
		NSLog(@"DEBUG: Setting number of DMX channels (lamps): %u (%u)", _numberOfChannels, _numberOfLamps);
#endif
	...
	if (!debugCount)
		[debugString appendString: @"Sending DMX Data:\n L : Red Gre Blu\n"];
#endif

	// put the rgb values in the appropriate place (determined by the lamp order)
	unsigned i, count = _numberOfLamps;
	unsigned lampOrderSize = [_lampOrder count];
	for (i = 0; i < count; ++i) 
	{
		NSNumber* lampOrder = (i < lampOrderSize) ? [_lampOrder objectAtIndex: i] : nil;
		unsigned lampIndex = (lampOrder) ? [lampOrder intValue] : -1;
...
#if DEBUG_DMX		
		if (!debugCount)
			[debugString appendFormat: @"%2u (%2u) : %3hhu %3hhu %3hhu\n", i, lampIndex, *rPtr, *gPtr, *bPtr];
#endif
		// DEBUG
//		if ((*rPtr > 0) || (*gPtr > 0) || (*bPtr > 0)) NSLog(@"DEBUG: %2u : %hhu %hhu %hhu", i, *rPtr, *gPtr, *bPtr);
	}
	
#if DEBUG_DMX	
	if (!debugCount) {
		NSLog(debugString);
		if (connectedToLanbox)
			NSLog(@"Sending to LanBox");
		else
			NSLog(@"No connection to LanBox (%@)", _address);
	}
#endif

	// TODO: switch to sendto
	// transmit packet
//	NSLog(@"DEBUG: About to send DMX");
	if (!connectedToLanbox) return;
	
	if (writev(_fd, &iov, 1) == -1) {
		NSLog(@"writev() failed %u", errno);
		return;
	}
}
