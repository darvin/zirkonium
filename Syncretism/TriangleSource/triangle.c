...
//  CR CHANGED -- renamed "index" to idx to prevent compiler warnings about
//	              shadowing the declaration of a function index in string.h
//  CR CHANGED -- moved the defines to the header
/* #define SINGLE */
...
//  CR CHANGED -- Want Triangle to act as a library
#define TRILIBRARY
...
#ifdef TRILIBRARY
      end1 = segmentlist[idx++];
      end2 = segmentlist[idx++];
      if (segmentmarkers) {
      ...
      *rlist = regionlist;
      ...
      for (i = 0; i < *regions; i++) {
        ...
        if (*stringptr == '\0') {
          ...
          regionlist[idx++] = (REAL) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...

{
  ...
  while (subsegloop.ss != (subseg *) NULL) {
    ...
    slist[idx++] = vertexmark(endpoint1);
    slist[idx++] = vertexmark(endpoint2);
    if (!b->nobound) {
      /* Copy the boundary marker. */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        dest(triangleloop, p2);
#ifdef TRILIBRARY
        elist[idx++] = vertexmark(p1);
        elist[idx++] = vertexmark(p2);
#endif /* TRILIBRARY */
        if (b->nobound) {
#ifndef TRILIBRARY
...
#else /* not TRILIBRARY */
          /* Write an infinite ray.  Edge number, idx of one endpoint, -1, */
